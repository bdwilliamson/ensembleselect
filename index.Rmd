...
The .blue1[augmentation set] allows for error control using a tuning parameter $k$

--

Define $V_n(\alpha) = \text{number of type I errors at level }\alpha$, $R_n(\alpha) = \lvert S_n(\alpha) \rvert$

Examples of more general error rates: 

--

generalized FWER: $gFWER(k) = Pr_{P_0}(V_n(\alpha) > k + 1), k \in \{0, \ldots, p - R_n(\alpha)\}$

--

proportion of false positives among the rejected variables at level $q \in (0, 1)$: $$PFP(q) = Pr_{P_0}(V_n(\alpha)/R_n(\alpha) > q)$$

--

false discovery rate: $FDR = E_{P_0}\{V_n(\alpha)/R_n(\alpha)\}$

--

$k$ can be determined to control one of these error rates! 
...
3. Use Rubin's rules to combine estimates, standard errors; use these to obtain p-values
...
* lasso (also with stability selection .small[ [Meinshausen and Buhlmann (2010)] ] and knockoffs .small[ [Barber and Candes (2015)] ])
...
* extrinsic selection (based on Super Learner; also with stability selection)

--

Two settings: (binary outcome, continuous features, varying missing data)
* linear outcome-feature relationship (setting A)
  * 6 important features (some very important, some weakly important)
  * $p \in \{30, 500\}$

--

* nonlinear outcome-feature relationship, correlated features (setting B)
  * 3 important features (all equally weakly important)
  * $p = 6$
  
---

## Numerical results: setting A

```{r results-a, echo = FALSE, fig.show = "hold", out.width = "140%", fig.align = "center"}
knitr::include_graphics(c(
    "img/binomial-probit-linear-normal-nested_select-perf.png"
  ))
```

---

## Numerical results: setting B

```{r results-b, echo = FALSE, fig.show = "hold", out.width = "140%", fig.align = "center"}
knitr::include_graphics(c(
    "img/nonlinear-normal-correlated_select-perf.png"
  ))
```
...
```{r data-results, echo = FALSE, fig.show = "hold", out.width = "140%", fig.align = "center"}
knitr::include_graphics(c(
    "img/data-analysis_mucinous_alg_1_covariates_0_biomarkers_1.png"
  ))
```
